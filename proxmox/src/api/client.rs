...
impl Client {
    ...
    /// Execute a PUT request expecting no response body
    ...
    /// Execute request with retry logic for operations that return no body
    async fn execute_delete_with_retry<F, Fut>(
        &self,
        request_fn: F,
        path: &str,
    ) -> Result<(), ApiError>
