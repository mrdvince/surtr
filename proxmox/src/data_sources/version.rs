...
use tfplug::{
    AttributeBuilder, Config, DataSource, Diagnostics, Dynamic, 
    SchemaBuilder, State,
};
...
impl VersionDataSource {
    ...
    pub fn schema_static() -> DataSourceSchema {
        SchemaBuilder::new()
            ...
            .attribute("version", AttributeBuilder::string("version").computed().description("Proxmox version"))
            .attribute("release", AttributeBuilder::string("release").computed().description("Proxmox release"))
            .attribute("repoid", AttributeBuilder::string("repoid").computed().description("Repository ID"))
            .build_data_source(0)
    }
}
...
impl DataSource for VersionDataSource {
    ...
    fn read(&self, _config: Config) -> tfplug::Result<(State, Diagnostics)> {
        ...
        values.insert("id".to_string(), Dynamic::String("proxmox_version".to_string()));
