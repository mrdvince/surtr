...
async fn provider_uses_env_vars_when_config_empty() {
    ...
    assert!(!read_diags.has_errors());
    assert_eq!(state.values.get("version").unwrap().as_string().unwrap(), "8.0.1");
...
async fn provider_prefers_config_over_env_vars() {
    ...
    let mut config_values = HashMap::new();
    config_values.insert("endpoint".to_string(), Dynamic::String(server.url()));
    config_values.insert("api_token".to_string(), Dynamic::String("config@pve!token=configsecret".to_string()));
    ...
    assert!(!read_diags.has_errors());
    assert_eq!(state.values.get("version").unwrap().as_string().unwrap(), "7.0");
